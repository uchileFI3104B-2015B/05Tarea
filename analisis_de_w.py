...
def una_iter(phi, phi_n, p): # = N_x, N_y, Lx, Ly, h, w = 1.
    ...
    N_x = int(N_x)
    N_y = int(N_y)
    for j in range(1, 12): #abajo de la linea
        for i in range(1, N_x -1): #recorre completo
            phi_n[i, j] = ((1 - w) * phi[i, j] +
                              w / 4 * (phi[i+1, j] + phi_n[i-1, j] +
                                       phi[i, j+1] + phi_n[i, j-1] ))

    for j in range(12, 14): #en la linea
        for i in range(1, 11): #para el borde antes de la linea
            phi_n[i, j] = ((1 - w) * phi[i, j] +
                              w / 4 * (phi[i+1, j] + phi_n[i-1, j] +
                                       phi[i, j+1] + phi_n[i, j-1] ))
        for i in range(41, N_x - 1): #borde despues de la linea
            phi_n[i, j] = ((1 - w) * phi[i, j] +
                              w / 4 * (phi[i+1, j] + phi_n[i-1, j] +
                                       phi[i, j+1] + phi_n[i, j-1] ))
        #esta parte es redundante, pero me sirve para ordenar mis ideas
        for j in range(12, 13): #bajo la linea, usando la condicion -1
            for i in range(11, 41):
                phi_n[i,j] =  ((1 - w) * phi[i, j] +
                          w / 3 * (phi[i+1, j] + phi_n[i-1, j] +
                                   phi_n[i, j-1] + h**2 * r(i, j, Lx, Ly, h)
                                   + h*(-1.)))


        for j in range(13, 14): #encima de la linea, usando condicion +1
            for i in range(11, 41):
                phi_n[i,j] =  ((1 - w) * phi[i, j] +
                          w / 3 * (phi[i+1, j] + phi_n[i-1, j] +
                                   phi_n[i, j-1] + h**2 * r(i, j, Lx, Ly, h)
    ...
    for j in range(14, 17): #sobre la linea y antes del rectangulo de la letra
        for i in range(1, N_x - 1):
            phi_n[i, j] = ((1 - w) * phi[i, j] +
                              w / 4 * (phi[i+1, j] + phi_n[i-1, j] +
                                       phi[i, j+1] + phi_n[i, j-1] ))
    for j in range(17, 58): #a la altura del rectangulo
        for i in range(1, 12): #a la derecha
            phi_n[i, j] = ((1 - w) * phi[i, j] +
                              w / 4 * (phi[i+1, j] + phi_n[i-1, j] +
                                       phi[i, j+1] + phi_n[i, j-1]))
        for i in range(12, 38): #dentro
            phi_n[i, j] = ((1 - w) * phi[i, j] +
                              w / 4 * (phi[i+1, j] + phi_n[i-1, j] +
                                       phi[i, j+1] + phi_n[i, j-1] +
                                       h**2. * r(i, j, Lx, Ly, h)))
        for i in range(38, N_x -1): #a la izq
            phi_n[i, j] = ((1 - w) * phi[i, j] +
                              w / 4 * (phi[i+1, j] + phi_n[i-1, j] +
                                       phi[i, j+1] + phi_n[i, j-1]))

    for j in range(58, N_y - 1): #sobre el rectangulo
        for i in range(1, N_x -1):
            phi_n[i, j] = ((1 - w) * phi[i, j] +
                              w / 4 * (phi[i+1, j] + phi_n[i-1, j] +
                                       phi[i, j+1] + phi_n[i, j-1] ))
...
def n_relajacion(w = 1, N = 5000):
    ...
    p= (N_x, N_y, Lx, Ly, h, w) #mas practico definir el vector para usar como
                                #parametro
